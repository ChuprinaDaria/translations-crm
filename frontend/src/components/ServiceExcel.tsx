...
import { KP, kpApi, serviceApi } from "../lib/api";
...
export function ServiceExcel() {
  ...
  useEffect(() => {
    const loadKPs = async () => {
      ...
      try {
        ...
        console.error(e);
        toast.error("Не вдалося завантажити КП для сервісу");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExport = async () => {
    if (selectedIds.length === 0) {
      toast.error("Оберіть хоча б одне КП для формування файлу сервісу");
      return;
    }

    setExporting(true);
    try {
      const blob = await serviceApi.exportService({
        kp_ids: selectedIds,
        format: "excel",
      });
      ...
      a.href = url;
      a.download = "service.xlsx";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      toast.success("Файл для сервісу згенеровано");
    } catch (e: any) {
      console.error(e);
      const message =
        e?.data?.detail ||
        e?.message ||
        "Не вдалося згенерувати файл для сервісу";
      toast.error(message);
    } finally {
      setExporting(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl text-gray-900 mb-2">Сервіс / Excel</h1>
        <p className="text-gray-600">
          Оберіть КП за датою, статусом або пошуком і згенеруйте файл для
          відділу сервісу (обладнання, послуги, транспорт для кожного заходу).
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2">
            <CardTitle>КП для сервісу</CardTitle>
            <InfoTooltip content="Виберіть один або кілька КП, які потрібно включити у файл для сервісу" />
          </div>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-4">
            ...
            >
              {exporting ? "Формування..." : "Згенерувати Excel для сервісу"}
            </Button>
          </div>
        </CardHeader>
        <CardContent>
