...
interface NotesManagerProps {
  ...
  onSave?: (notes: Note[]) => void | Promise<void>;
  /** Callback для завантаження нотаток (якщо не використовується localStorage) */
  onLoad?: () => Promise<Note[]> | Note[];
  /** Callback при додаванні нотатки (для синхронізації з API). Може повернути оновлену нотатку з правильним ID */
  onAddNote?: (note: Note) => void | Promise<void> | Promise<Note> | Note;
  /** Callback при видаленні нотатки (для синхронізації з API) */
...
export function NotesManager({
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      // Якщо є onAddNote callback, викликаємо його для синхронізації з API
      let finalNote = newNoteObj;
      if (onAddNote) {
        const result = await Promise.resolve(onAddNote(newNoteObj));
        // Якщо callback повернув оновлену нотатку з правильним ID, використовуємо її
        if (result && typeof result === 'object' && 'id' in result) {
          finalNote = result as Note;
        }
      }

      const updatedNotes = [...notesList, finalNote];
      setNotesList(updatedNotes);
      setNewNote('');
      
      // Якщо немає onAddNote, зберігаємо через onSave або localStorage
      if (!onAddNote) {
  ...
  const handleRemoveNote = async (noteId: string) => {
    ...
    try {
      ...
      // Якщо є onRemoveNote callback, викликаємо його для синхронізації з API
      if (onRemoveNote) {
        await Promise.resolve(onRemoveNote(noteId));
      } else {
        // Інакше зберігаємо через onSave або localStorage
