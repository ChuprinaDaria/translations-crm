...
- **500 Internal Server Error** - Помилка сервера CRM

### З боку Sloth (RAG бота):
- **400 Bad Request** - `"Template error: 'ai' is undefined"` 
  - **Причина:** Помилка в Jinja2 шаблоні Sloth
  - **Рішення:** Перевірте конфігурацію Sloth, змінна `ai` має бути визначена в контексті шаблону
  - **Це НЕ помилка CRM** - проблема на стороні Sloth

## 🐛 Troubleshooting

### Помилка: "Template error: 'ai' is undefined"

**Проблема:** Sloth використовує Jinja2 шаблони і змінна `ai` не передана в контекст.

**Де виправляти:** У Sloth, не в CRM.

**Перевірте в Sloth:**
```python
# У вашому Sloth коді шаблон може виглядати так:
template = """
Відповідь: {{ ai.response }}  # ❌ Помилка: 'ai' не визначено
"""
...
**Можливі рішення:**
1. Додайте змінну `ai` в контекст шаблону Sloth
2. Змініть шаблон щоб не використовувати `ai.*`
3. Перевірте конфігурацію Sloth `config.yaml` або `prompts.yaml`
